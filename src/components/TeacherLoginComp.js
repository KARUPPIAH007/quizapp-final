...
function TeacherLoginComp() {
  ...
  const handleSubmit = (event) => {
    ...
    axios.get("http://localhost:8888/teacher").then((res)=>{
      ...
      const data = userData.filter((val)=>{return val.teacher_email===email && val.teacher_password===password})
      if(data.length >0){
          sessionStorage.setItem("email",email);
          sessionStorage.setItem("loggedin",true);
          nav("/maindashboard");//Change here 
      }
      else{
      ...
      <LoginNavComp></LoginNavComp>
        ...
        >
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
            ...
            <Grid container>
              <Grid item>
                <Link href="/teachersignup" variant="body2">
