...
//       // await axios.patch(`http://localhost:3000/tests/${examId}`, { status: isChecked });
...
//       await axios.delete(`http://localhost:8888/tests/${examId}`);

//       // Update the exams state to remove the deleted exam
//       setExams(exams.filter(exam => exam.examId !== examId));

//       // Also update the selectedExams state if the deleted exam was selected
//       const updatedSelectedExams = selectedExams.filter(id => id !== examId);
//       setSelectedExams(updatedSelectedExams);
//       localStorage.setItem('selectedExams', JSON.stringify(updatedSelectedExams));
//     } catch (error) {
...
//                 </td>
//                 <td>
//                   <Link to="/maindashboard/editquestioncomponent" variant="body2">Edit</Link>
//                   {' '}
//                   <button 
